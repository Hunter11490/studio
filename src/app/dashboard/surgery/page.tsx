...
export default function SurgeryPage() {
  ...
  const surgeryDoctors = useMemo(() => {
    return doctors.filter(d => d.specialty === 'General Surgery');
  }, [doctors]);
  
  const partnerDoctors = useMemo(() => {
    return surgeryDoctors.filter(d => d.isPartner)
  }, [surgeryDoctors]);

  const filteredAndSortedDoctors = useMemo(() => {
    
    const sorted = [...surgeryDoctors].sort((a: Doctor, b: Doctor) => {
      switch (sortOption) {
        case 'name':
  ...
  }, [surgeryDoctors, searchTerm, filterPartners, sortOption]);
