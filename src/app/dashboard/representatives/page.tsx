...
import { Frown, UserSearch, Pencil, Trash2 } from 'lucide-react';
...
function RepresentativeRow({ doctor }: { doctor: Doctor }) {
    const { t } = useLanguage();
        ...
        return (
             <div className="border rounded-lg w-full bg-card">
                <Table>
                    ...
                    <TableBody>
                        {filteredDoctors.map(doctor => <RepresentativeRow key={doctor.id} doctor={doctor} />)}
                    </TableBody>
                </Table>
            </div>
        );
